...
-   `callOptions` **[Object][4]** Call options.
    ...
    -   `callOptions.customParameters` **[Array][9]&lt;{CustomParameter}>?** Custom SIP header parameters for the SIP backend
...
These headers can be specified with the [call.make][14] and [call.answer][15] APIs.
They can also be set on a call using the [call.setCustomParameters][16], and sent using the [call.sendCustomParameters][17] API.

A Call's custom parameters are a property of the Call's [CallObject][18],
...
Some call operations cannot be performed while the call is on hold. The
   call can be taken off hold with the [call.unhold][25] API.
...
The SDK will emit a [call:stateChange][27]
   ...
   [call:stateChange][27] event as well.
...
The custom parameters set on a call can be sent directly with the [call.sendCustomParameters][17] API.
...
To change or remove the custom parameters on a call, use the [call.setCustomParameters][16] API.
...
Returns **[Array][9]&lt;[call.CallObject][28]>** Call objects.
...
Returns **[call.CallObject][28]** A call object.
...
The SDK will stop any/all local media associated with the call. Events
   ...
   [call:trackEnded][29] event for more
   information.
...
The SDK will emit a [call:stateChange][27]
   ...
   [call:stateChange][27] event, that the
   call was ended.
...
The SDK will emit a [call:newTrack][30] event
...
The SDK will emit a [call:trackEnded][29]
...
The [call.removeMedia][32] API can be used to perform the same
   behaviour as `stopVideo`. [call.removeMedia][32] is a
   ...
   when using [call.removeMedia][32] will perform the same behaviour
   as using `stopVideo`.
...
The [call.removeMedia][32] API can be used to perform the same
   behaviour as `stopScreenshare`. [call.removeMedia][32] is a
   general-purpose API for removing media from a call, which covers the
   same functionality as `stopScreenshare`. Specifying only the screen
   track when using [call.removeMedia][32] will perform the same
...
The SDK will emit a
   [call:statsReceived][34] event, after
...
The devices used for a call can be selected as part of the APIs for
   ...
   [call.make][14] and [call.answer][15] APIs, and speaker can be
   chosen when the audio track is rendered with the
   [media.renderTracks][36] API.
...
The same behaviour as the `changeSpeaker` API can be implemented by
   ...
   re-rendering it with a new speaker with [media.renderTracks][36].
...
The 'media' namespace provides an interface for interacting with Media that the
   SDK has access to. Media is used in conjunction with the [Calls][40]
...
The Media feature also keeps track of media devices that the user's machine
   ...
   the [media.getDevices][41] API.

### getDevices

Retrieves the available media devices for use.

The [devices:change][42] event will be
...
The SDK will emit a [media:muted][43] event
...
The SDK will emit a [media:unmuted][44] event
...
[16]: #callsetcustomparameters

[17]: #callsendcustomparameters
...
[27]: #calleventcallstatechange

[28]: #callcallobject

[29]: #calleventcalltrackended

[30]: #calleventcallnewtrack
...
[34]: #calleventcallstatsreceived

[35]: #calleventcalltrackreplaced

[36]: #mediarendertracks
...
[42]: #mediaeventdeviceschange

[43]: #mediaeventmediamuted

[44]: #mediaeventmediaunmuted

[45]: #callsdphandlerfunction
