...
call can be taken off hold with the [call.unhold][49] API.
...
Set the [Custom Parameters][52] of a Call, to be provided to the remote endpoint.
...
To change or remove the custom parameters on a call, use the [call.setCustomParameters][45] API.
...
Returns **[Array][13]<[call.CallObject][53]>** Call objects.
...
*   `options` **[Object][7]?**  (optional, default `{}`)
    ...
    *   `options.dscpControls` **[call.DSCPControls][56]?** Options for configuring DSCP markings on the media traffic
...
Can be retrieved using the [connection.getSocketState][78] API.
...
Returns **[connection.WSConnectionObject][79]** Details about the current websocket connection, including state and configuration.
...
[config.logs][80]) to customize how the logs are
...
For device information, the [media.getDevices][82] API will retrieve
...
The SDK will emit a [devices:change][83]
...
The SDK will emit a [media:muted][87] event
...
The SDK will emit a [media:unmuted][88] event
...
A Track can be muted using the [media.muteTracks][90] API.
...
standard and/or voip push notifications. It can then send these notifications to the SDK with [api.notifications.process][94]
...
Returns **[Promise][95]** When successful,  the information of the registration.
...
Returns **[Promise][95]** When successful, the promise will resolve with undefined.
Promise will reject with error object otherwise.
...
This is a wrapper on top of the browser's [fetch API][96]
...
*   `init` **RequestInit** An object containing any custom settings that you want to apply to the request. See [fetch API][96]
...
Returns **[Promise][95]<[Response][97]>** A promise for a [Response][98] object.
...
[78]: #connectiongetsocketstate

[79]: #connectionwsconnectionobject
...
[87]: #mediaeventmediamuted

[88]: #mediaeventmediaunmuted

[89]: #callmediaconstraint
...
[94]: api.notifications.process

[95]: https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Promise

[96]: https://developer.mozilla.org/en-US/docs/Web/API/fetch

[97]: https://developer.mozilla.org/docs/Web/Guide/HTML/HTML5
