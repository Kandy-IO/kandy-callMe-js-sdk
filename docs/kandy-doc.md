...
*   `tracks` **[Array][13]<[call.TrackObject][31]>** A list of Track objects that are contained in this Media object.
...
These parameters can be specified with the [call.make][33] and [call.answer][34] APIs.
They can also be set after a Call is established using the [call.setCustomParameters][35] API, and sent using the [call.sendCustomParameters][36] API.
...
A Call's custom parameters are stored on the Call's [CallObject][38],
...
part of the CallObject and are only provided via the [call:customParameters][37] event.
...
Set the [Custom Parameters][42] of a Call, to be provided to the remote endpoint.
...
*   `options` **[Object][7]?**  (optional, default `{}`)
    ...
    *   `options.dscpControls` **[call.DSCPControls][46]?** Options for configuring DSCP markings on the media traffic
...
The Call's media connection state is a property of the [CallObject][38],
which can be retrieved using the [call.getById][24] or
[call.getAll][23] APIs.
...
the [call.removeMedia][48] API. Tracks are also removed from Calls
...
its media removed, see [call:removedMedia][64]
...
See the [call.getStats][65] API for more information.
...
*   `params` **[Object][7]** 
    ...
    *   `params.result` **[Map][66]** The RTCStatsReport.
    *   `params.error` **[api.BasicError][19]?** An error object, if the operation was not successful.
...
A Track is replaced by the local user using the [call.replaceTrack][61]
API.
...
*   `params` **[Object][7]** 
    ...
    *   `params.oldTrack` **[call.TrackObject][31]?** State of the replaced track.
    *   `params.error` **[api.BasicError][19]?** An error object, if the operation was not successful.
...
This event is emitted as a result of the [call.getAvailableCodecs][67] API. Please refer to the API for more
information.
...
Can be retrieved using the [connection.getSocketState][68] API.
...
[MediaConstraint(s)][79] that was provided.
...
A Track can be muted using the [media.muteTracks][80] API.
...
A Track can be unmuted using the [media.unmuteTracks][81]
API.
...
[media:sourceMuted][83] event), and will
...
standard and/or voip push notifications. It can then send these notifications to the SDK with [api.notifications.process][84]
...
Returns **[Promise][85]** When successful,  the information of the registration.
...
Returns **[Promise][85]** When successful, the promise will resolve with undefined.
Promise will reject with error object otherwise.
...
This is a wrapper on top of the browser's [fetch API][86]
...
*   `init` **RequestInit** An object containing any custom settings that you want to apply to the request. See [fetch API][86]
...
Returns **[Promise][85]<[Response][87]>** A promise for a [Response][88] object.
...
initialization of the SDK using the [config.call.sdpHandlers][89] configuration
parameter. They can also be set after the SDK's creation by using the
[call.setSdpHandlers][90] function.
...
[37]: call.event:call:customParameters

[38]: #callcallobject

[39]: #callunhold

[40]: #calleventcalloperation

[41]: #calleventcallstatechange

[42]: #callcustomparameter
...
[63]: #mediagettrackbyid

[64]: #calleventcallremovedmedia

[65]: #callgetstats

[66]: https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Map

[67]: #callgetavailablecodecs

[68]: #connectiongetsocketstate
...
[79]: #callmediaconstraint

[80]: #mediamutetracks

[81]: #mediaunmutetracks

[82]: #mediaeventmediasourceunmuted

[83]: #mediaeventmediasourcemuted

[84]: api.notifications.process

[85]: https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Promise

[86]: https://developer.mozilla.org/en-US/docs/Web/API/fetch

[87]: https://developer.mozilla.org/docs/Web/Guide/HTML/HTML5

[88]: https://developer.mozilla.org/en-US/docs/Web/API/Response

[89]: #configconfigcall

[90]: #callsetsdphandlers
